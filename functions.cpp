...
extern size_t EXIT_COND;
    ...
    long long hash_calc (my_stack *head) {
        ...
        if (head->arr != nullptr) {
            hash ^= *((long long*)(head->arr - sizeof (long long) / sizeof (stack_t)));
    ...
    static void verificator (my_stack *head) {
        ...
        if (head->arr == nullptr && head->size_array != 0) {
            ...
            if (*((long long*)(head->arr - sizeof (long long) / sizeof (int))) != alive || 
...
void constructor (my_stack *head, int size_array) {
    ...
    *((long long*)(head->arr + head->size_array)) = alive;
#else
    head->arr = (stack_t*)calloc(head->size_array, sizeof (stack_t));
#endif
    }
...
void push (my_stack *head, stack_t value) {
    ...
    if (head->error == STACK_OVERFLOW) {
        ...
        if (head->size_array > 0)
            head->arr = (stack_t*) realloc (head->arr, sizeof (stack_t) * head->size_array);
...
stack_t pop (my_stack *head) {
    ...
    if (EXIT_COND != 1) {
        ...
        stack_t tmp = *(head->arr + head->size_stack);
